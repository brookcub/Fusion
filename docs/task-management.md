...
4. **in-review** — implementation complete; awaiting finalization
   ...
   - Automatic merge recovery has separate durable budgets for conflict retries (`mergeRetries`), direct transient retries (`mergeTransientRetryCount`), and exhausted transient recovery (`mergeDetails.transientRecoveryCount`). Re-entry and restart retain the applicable counter rather than granting a new episode. Recovery eligibility uses increasing delays (60 seconds, then doubling to a five-minute maximum) and stops at its cap; a failed review card remains in place with its worktree, commits, and completed steps preserved for operator repair.
   - When repeated ownership evidence cannot verify an already-merged candidate, its recovery diagnostic does not create a new stall episode. The resulting terminal stall report is emitted once for an unchanged cause, while a materially changed cause can still be surfaced. After the underlying condition is repaired, use the supported Retry action to begin a fresh explicit episode; automatic recovery never impersonates that action.
