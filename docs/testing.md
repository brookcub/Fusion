...
**Static-validator and lane ordering:** `test:gate:static` declares the canonical, directly runnable read-only validators. `scripts/run-merge-gate.mjs` invokes the static runner and requires **every** validator to succeed before starting the engine-core, PostgreSQL, and core-unit lanes concurrently. It uses Node argument arrays, not POSIX shell syntax, so `pnpm test:gate` also works from Windows paths containing spaces. Every lane must settle successfully before CI-shape runs. Package scripts own the exact test selectors; the launcher does not carry a second reduced inventory.

**Private PostgreSQL gate:** the public merge gate creates its own temporary home and embedded database before running the canonical PostgreSQL canaries. It does not use an ambient `DATABASE_URL` or the live Fusion instance. Setup, tests, and shutdown must all succeed; an empty or skipped suite is failure. The printed evidence directory retains the test report and logs for diagnosis. Other explicit PostgreSQL development commands below remain available independently.
