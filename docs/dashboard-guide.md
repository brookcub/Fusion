...
<!-- FNXC:CompletionEvidence 2026-09-09-10:36: FUSI-007 requires every card, list layout, and task-detail status to distinguish a landed merge from verified completion without implying that a failed required post-merge check undoes the landing. -->
- A landed task can show a separate verification badge in Board, List, and task detail. **Verified** means current required evidence has passed; **Failed**, **Pending**, **Stale**, and **Unavailable** mean the commit remains landed but cannot be presented as verified complete. Advisory post-merge failures remain visible as warnings and do not block verified completion. Stale and unavailable labels direct operators to current settings or proof rather than treating an older passing receipt as success.
