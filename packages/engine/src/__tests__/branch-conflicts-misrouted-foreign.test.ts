...
describe("classifyMisroutedForeignCommit", () => {
  it("classifies trailer-attributed .changeset-only commit as misrouted", async () => {
    ...
    try {
      ...
      git(dir, "add", ".changeset/fn-1234-fix.md");
      git(dir, "commit", "-m", "chore: changeset only", "-m", "Fusion-Task-Id: FN-1234");
  ...
  it("classifies subject-only task attribution and normalizes case", async () => {
    ...
    try {
      ...
      git(dir, "add", ".changeset/fn-7777-feature.md");
      git(dir, "commit", "-m", "feat(fn-7777): add feature");
      const sha = git(dir, "rev-parse", "HEAD");
  ...
  it("does not classify misrouted when shared paths are present", async () => {
    ...
    try {
      ...
      git(dir, "add", ".changeset/fn-4321-fix.md", "packages/engine/src/executor.ts");
      git(dir, "commit", "-m", "fix(FN-4321): mixed paths", "-m", "Fusion-Task-Id: FN-4321");
      const sha = git(dir, "rev-parse", "HEAD");
  ...
  it("does not classify when attribution matches current task", async () => {
    ...
    try {
      ...
      git(dir, "add", ".changeset/fn-9000-fix.md");
      git(dir, "commit", "-m", "test(FN-9000): same task");
      const sha = git(dir, "rev-parse", "HEAD");
