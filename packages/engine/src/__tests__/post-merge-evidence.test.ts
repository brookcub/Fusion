...
it("delivers the landed candidate's result rather than an old failure or model text", () => {
  const task = fixture();
  expect(evidence(task).candidates).toMatchObject([{ candidateSha: sha, evidence: "passed", checks: [{ exitCode: 0 }] }]);
  expect(buildPostMergeEvidence(task, settings)).not.toContain("old check failed");
});
it.each(["candidate", "source", "settings", "command", "pending", "nonzero", "malformed", "missing"])("never claims success for %s evidence", kind => {
  ...
  expect(evidence(task).candidates[0].evidence).toBe("unavailable");
});
...
it("distinguishes no commands from a passing check", () => {
  ...
  expect(evidence(task, {} as any).candidates[0].evidence).toBe("not-run");
});
it("requires coverage of every workspace landing", () => {
  ...
  expect(evidence(task).candidates.map((r: any) => r.evidence)).toEqual(["passed", "unavailable"]);
});
it("does not share one passing check between repositories at the same commit", () => {
  ...
  expect(evidence(task).candidates).toMatchObject([
    { repository: "one", candidateSha: sha, evidence: "unavailable" },
    { repository: "two", candidateSha: sha, evidence: "unavailable" },
  ]);
});
