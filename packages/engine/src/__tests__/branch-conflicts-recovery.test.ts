...
describe("branch contamination recovery classification", () => {
  ...
  async function makeCommit(repoDir: string, body: string, subject: string, foreignTaskId: string, file = "note.txt"): Promise<BranchCrossContaminationCommit> {
    ...
    await git(repoDir, "commit", "-m", subject, "-m", `Fusion-Task-Id: ${foreignTaskId}`);
  ...
  it("classifies all foreign commits as already-upstream when patches exist on main", async () => {
    ...
    await git(repoDir, "cherry-pick", commit.sha);
  ...
  it("classifies mixed foreign commits into already-upstream and unique buckets", async () => {
    ...
    await git(repoDir, "checkout", "main");
    await git(repoDir, "cherry-pick", upstreamCommit.sha);
    await git(repoDir, "checkout", "feature");
  ...
  it("does not classify bootstrap misbinding when an own-task commit exists", async () => {
    ...
    await git(repoDir, "add", "note.txt");
    await git(repoDir, "commit", "-m", "feat(FN-4488): own work", "-m", "Fusion-Task-Id: FN-4488");
  ...
  it("does not classify bootstrap misbinding when non-attributed commits are present", async () => {
    ...
    await git(repoDir, "add", "note.txt");
    await git(repoDir, "commit", "-m", "refactor: unattributed cleanup");
  ...
  it("reanchors branch to base and clears bootstrap foreign history", async () => {
    ...
    const before = await git(repoDir, "rev-parse", "feature");
    ...
    const after = await git(repoDir, "rev-parse", "feature");
    const range = await git(repoDir, "rev-list", "--count", `${baseSha}..feature`);
  ...
  it("reanchors without checkout -B when detached worktree is already at base on bound branch", async () => {
    ...
    await git(repoDir, "worktree", "add", "--detach", secondaryWorktree, baseSha);

    await expect(
      git(secondaryWorktree, "checkout", "-B", "feature", baseSha),
    ...
    const headSha = await git(secondaryWorktree, "rev-parse", "HEAD");
  ...
  it("treats already-bound branch-at-base as a no-op fast-path", async () => {
    ...
    const currentBranch = await git(repoDir, "symbolic-ref", "--quiet", "--short", "HEAD");
    expect(result.previousTipSha).toBe(baseSha);
    expect(result.newTipSha).toBe(baseSha);
  ...
  it("auto-recovers by dropping already-upstream foreign commits and preserving remaining branch work", async () => {
    ...
    await git(repoDir, "add", "own.txt");
    await git(repoDir, "commit", "-m", "feat(FN-4428): own work", "-m", "Fusion-Task-Id: FN-4428");

    await git(repoDir, "checkout", "main");
    await git(repoDir, "cherry-pick", foreign.sha);
    await git(repoDir, "checkout", "feature");

    const originalTip = await git(repoDir, "rev-parse", "HEAD");
    ...
    const history = await git(repoDir, "log", "--format=%s", `${baseSha}..feature`);
