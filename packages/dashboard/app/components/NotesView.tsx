...
export function NotesView({ projectId, addToast }: NotesViewProps) {
  ...
  const handleSelect = async (id: string) => { if (id !== notes.selected?.id && await abandon()) await notes.select(id); };
  ...
  return <section className={`notes-view${notes.selected ? " notes-view--detail" : ""}`} aria-label={t("nav.notes", "Notes")}>
    ...
    <div className="notes-layout">
      <aside className="notes-list" aria-label={t("notes.list", "Notes list")}>
        ...
        <div className="notes-list-items">{notes.notes.map((note) => <button key={note.id} type="button" className={`notes-list-item${note.id === notes.selected?.id ? " active" : ""}`} aria-current={note.id === notes.selected?.id ? "page" : undefined} onClick={() => void handleSelect(note.id)}><strong>{note.title}</strong><time dateTime={note.updatedAt}>{new Date(note.updatedAt).toLocaleString()}</time></button>)}</div>
      </aside>
      <main className="notes-detail">
        {notes.selected ? <>
          ...
          {notes.error && !notes.conflict ? <div className="notes-error" role="alert">{notes.error}<button className="btn" type="button" onClick={() => void handleSave()}>{t("common.retry", "Retry")}</button></div> : null}
